<template>
  <div class="admin-profile animate-fade-in">
    <div class="profile-hero">
      <div class="profile-hero-bg admin-bg"></div>
      <div class="profile-hero-content">
        <div class="profile-avatar-lg admin-avatar">
          <span>{{ auth.user?.name?.charAt(0) || 'A' }}</span>
        </div>
        <div class="profile-hero-info">
          <h2 class="heading-md">{{ auth.user?.name || 'Administrator' }}</h2>
          <p class="text-sm text-muted">{{ auth.user?.email || '' }}</p>
          <span class="role-badge admin-badge">Administrator</span>
        </div>
      </div>
    </div>
    ...
    <div class="profile-section" v-if="tab === 'info'">
      ...
      <div class="form-grid">
        ...
        <div class="input-group">
          <label>Role</label>
          <input value="Platform Administrator" class="input" disabled />
        </div>
      </div>
...
onMounted(async () => {
  try {
    ...
    form.name = data.name || ''; form.email = data.email || ''; form.phone = data.phone || ''
  } catch (e) {
...
async function saveProfile() {
  try {
    await profileApi.update({ name: form.name, email: form.email, phone: form.phone })
    alert('Profile saved!')
...
.admin-bg { height: 100px; background: linear-gradient(135deg, #6366F1, #4F46E5); }
...
.profile-tab.active {
  background: var(--color-surface-raised); color: #6366F1;
